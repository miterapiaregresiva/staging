...
const archive=(data.archive_copies||[]).map(x=>`<li><strong>${esc(x.label)} · ${esc(x.language)}</strong><br><span class="catalog-sub">Internet Archive · préstamo o consulta digital según disponibilidad</span><a href="${esc(x.url)}" target="_blank" rel="noopener noreferrer">Leer online en Internet Archive</a></li>`).join('') || '<li>No hemos verificado todavía un ejemplar digital de esta obra en Internet Archive.</li>';
const relatedCards = items => (items||[]).map(x => {
  ...
  return `<article class="card"><div class="book-cover image-with-credit"><img src="${esc(relatedCover)}" alt="${hasRelatedCover ? 'Portada de ' : 'Portada no disponible de '}${esc(x.title)}" loading="lazy">${relatedCredit}</div><h3><a href="/autores/${authorSlug}/libros/${esc(x.slug)}/">${esc(x.title)}</a></h3><p><strong>Original:</strong> <em>${esc(x.original_title)}</em>.</p></article>`;
}).join('');
...
const values={
  ...
  SLUG:esc(data.slug), LEAD:esc(data.lead), ABOUT:paragraphs, ISBNS:isbns, COVER_SRC:esc(cover.src), COVER_ALT:esc(cover.alt), COVER_CREDIT:coverCredit,
  ARCHIVE:archive, DATA_SOURCE:esc(data.data_source||`/data/library/${authorSlug}.json`),
