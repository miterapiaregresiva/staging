...
def update_catalog(works: dict, checked_at: str) -> dict:
...
def update_lazos(works: dict, records: list[dict], checked_at: str) -> dict | None:
    ...
    data["availability_source"] = "/data/availability/brian-weiss.json"
...
def main() -> int:
    ...
    for index, (work_slug, bica_id) in enumerate(tasks, start=1):
    ...
    write_json(CATALOG_PATH, update_catalog(works, checked_at))
    lazos = update_lazos(works, records, checked_at)
